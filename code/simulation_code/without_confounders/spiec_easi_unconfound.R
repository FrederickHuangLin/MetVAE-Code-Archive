...
res_sim = foreach(i = simparams_list, .verbose = TRUE, .combine = rbind, .packages = c("MASS", "SpiecEasi"),
  ...
  # SPIEC-EASI
  se_mb = spiec.easi(data = data_miss, method = "mb", lambda.min.ratio = 1e-2,
                     nlambda = 20, pulsar.params = list(rep.num = 50))
  est_cor = as.matrix(symBeta(getOptBeta(se_mb), mode = "maxabs"))
...
write_csv(data.frame(res_sim), "sim_spiec_easi_unconfound.csv")